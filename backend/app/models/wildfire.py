...
SCALER_PATH = MODELS_DIR / "wildfire_scaler.pkl"
...
def train_model():
    ...
    temperature_c = rng.uniform(10, 50, n_samples)
    humidity_pct = rng.uniform(5, 95, n_samples)
    wind_speed_kmh = rng.uniform(0, 100, n_samples)
    drought_index = rng.uniform(0, 10, n_samples)
    vegetation_density = rng.uniform(0, 1, n_samples)
    distance_to_road_km = rng.uniform(0, 50, n_samples)

    X = np.column_stack([
        temperature_c, humidity_pct, wind_speed_kmh,
        drought_index, vegetation_density, distance_to_road_km
    ])

    risk = (
        (temperature_c > 35).astype(float) * 0.4 +
        (humidity_pct < 20).astype(float) * 0.35 +
        (wind_speed_kmh > 40).astype(float) * 0.25
    )
